...
**[STORY.md](docs/STORY.md)** — Full technical article: detection methods, reverse-engineered protocols, SIDFX secondary probing, self-modifying code patterns, hardware testing methodology, and lessons learned. For sceners, SID musicians, and hardware developers.

---

## Screenshot

![SID Detector running in VICE](screenshot.png)
...
`make ci-full` runs 46 unit tests + a 30-case variant sweep (currently 30/30) that byte-diffs
...
- **Fixed V1.5.08:** An ARMSID or SwinSID Ultimate at `$D5xx–$D7xx` reported as a plain `8580 FOUND` instead of by name. The DIS probe that names it was never reached: the two stereo mirror tests (`s_s_arm_chk`'s oscillator cross-read and `s_s_arm_mir_test`) both asked *"does the candidate's OSC3 read zero?"* when the question they need to answer is *"does the reference oscillator show through here?"* — and a silenced SID does **not** read 0 at `+$1B`, it keeps the residue of whatever the oscillator last produced (`u64_fingerprint_scan` drives every `$D5xx` candidate before these sweeps run). Measured at the rejection point: `A=$AA` on one run and `A=$D8` on another with `X` still `$18`, i.e. the very first read; adding the ~1280-cycle settle that `s_s_arm_mir_wait` uses does not help, because it is residue rather than write latency. Both tests now sample the candidate **while nothing is driving it** and look for a *change* against that baseline. Two earlier attempts at exactly this fix were reverted because they destabilised the `$D4xx` window scan — in a single-SID config every `$D4xx` address mirrors `$D400` and ~20 extra cycles were enough to move the reported mirror from `$D420` to `$D460`, which would be wrong on a real FPGASID (its SID2 genuinely is at `$D420`). The difference this time is placement: the baseline read sits *before* the reference oscillator is started, so the instruction sequence between that start and the first candidate read is byte-for-byte what it was, and the extra cycles land where nothing is being sampled. Result: `D500 ARMSID FOUND`, `D500 SWINSID ULTIMATE FOUND`, and `tri-D420-armsid+D500-armsid` r18 corrected too — variant sweep **28/30 → 30/30**, with no `$D460` row in the full sweep or in 24 targeted runs of the six single-SID cases that reach this code. Details and the v6/v7-vs-V1.5.08 comparison: [CODE-REVIEW.md](CODE-REVIEW.md) § P0-5.
- **Fixed V1.5.06:** Secondary SIDs in `$D4xx–$D7xx` were mislabelled as U64 UltiSID on every machine. `u64_fingerprint_scan` ran for any real-SID primary with **no `is_u64` gate** and stamped an ULTISID curve code (`$20`/`$22`) on every independent slot it found; because it runs *before* the family-specific `sidstereostart` sweeps, their address dedup then found the slot already present and never refined it. On a plain C64 an ordinary second 8580, an ARMSID or a SwinSID Ultimate in a stereo cartridge all reported as a U64 UltiSID — reproducible with no chip personality at all (`tri-D500+DE00-plain-8580`: D500 read `8580 INT`, DE00 — outside the scan range — read `8580 FOUND`). The V1.4.37 gate removal was deliberate and hardware-verified for Tuneful Eight, so it is not restored; instead the scan now **discovers provisionally and lets the family scans refine**: it stores the type its own `checkrealsid` measured plus a per-slot `sid_prov` flag, `s_s_dup_lp` and `fll_dup_lp` (via a new `fll_refine` entry point) overwrite the type **in place** while that flag is set, and `ufs_chk_u64` converts the provisional entries back to the ULTISID codes whenever `is_u64` is set — so Tuneful Eight is unchanged. Fixing this exposed **two self-comparison bugs**: once a slot carried a real `$01`/`$02` type, both mirror checks began testing the candidate *against itself* (driving its own voice 3 and reading its own OSC3 always returns non-zero), so it rejected itself; `s_s_arm_mir_test` and `fll_mlp` now skip any list entry at the candidate's own address. Also fixed: an **IRQ-vector race in `readkey2`** (the two-byte `$0314/$0315` write happened with interrupts enabled and the KERNAL CIA timer still live — an IRQ landing between the writes dispatched through a half-updated vector), `Checkarmsid`'s `cas_d41d7` missing from its self-mod patch list, a Q-page vs main-screen disagreement on the ULTISID `$22`/`$23` split, `arm2sid_populate_sid_list` being the only `sid_list` writer with no bounds guard, and 3 dead routines carrying latent `cmp $07` (zero-page compare where `#$07` was meant) removed — 273 lines / ~364 bytes. The 19 print-dispatch sites that parked X in the **stack pointer** across `jsr $E50C` now use a save byte (one of them never restored it at all, which is why `readkey2` had to repair SP). Variant sweep 21/30 → **28/30**; unit suite 43 → **46** tests; 23 new host-side Python tests (26 as of the P3-5 work). Full review, measurements and what was deliberately left undone: [CODE-REVIEW.md](CODE-REVIEW.md).
